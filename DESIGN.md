...
Spending shows an opaque white monthly-budget CTA card whenever no budget is configured, whether or not trips have already been recorded. A configured zero budget is still a budget. Its Set monthly budget action opens the existing Settings budget sheet through a consumed `edit=budget` entry request. Keep the shared amount field, validation, save and error handling; do not invent another budget editor.

### Plan quick check

- Keep `TabLargeTitle`, `CollapsibleTabHeader`, the avatar action and the existing four-tab navigation. Do not import prototype household-name banners or navigation chrome.
- Show one catalogue-backed product card with up to two decorative stack layers. Right swipe means **Need this** (`add`); left means **Still have it** (`still_have_some`), not deleting a shopping item or stopping Pantry tracking. Keep accessible buttons and **Not sure · ask me later** (`not_this_time`).
- Stack layers use 10pt upward offsets and matching horizontal insets, with reserved top space and opaque warm surfaces. Do not uniformly scale a tall back card: shrinking its height cancels the top offset and hides the stack. Scope the layers to the card itself, excluding the swipe hints. Show one layer for two remaining products and two layers for three or more; none for the last product. There is no standalone View list button underneath an active check stack; retain the completion-state action.
- Use the shared compact coral `ProgressBar`, without a visible numeric count. A review session freezes its candidate IDs; live household updates can resolve cards without inserting new cards into that session’s denominator. New suggestions are offered separately.
- Below the review, use the compact `NextShopCard`: a coral date tile, actual list name/count, optional planned spend/budget and a date/time/mode summary. Tapping the card opens the Shop tab. Its separate pencil action opens `NextShopScheduleSheet` with a calendar, household-local 24-hour time and In store/Online selection; Save commits them together, Cancel discards the draft. Keep the card and edit targets as siblings so editing never also navigates to Shop. Do not restore the tall legacy card, duplicate Start shopping CTA or Saturday-only scheduling shortcut. Keep next-list selection, other lists and creation. Empty-state **View list** actions navigate directly to Shop. **Choose a few regulars** navigates directly to Pantry.
- Match the approved prototype’s schedule strip: white 18pt-radius surface, 14pt padding, small coral date tile, 17pt title and compact metadata in one row, with a coral pencil edit action (44pt target). Do not add a duplicate “Next shop” eyebrow or a separate metadata footer. Keep date/time, shopping mode and any planned amount visible without fixed-height text clipping.
- Do not add a standalone **View list** button below the schedule strip: the card opens Shop, and empty-state View list actions remain available. **Other lists** follows only when secondary lists exist, with their real count, compact white 78pt-minimum rows, warm calendar/bag icon tiles and a trailing chevron; opening a row goes to its own list. Hide the heading/count/rows when there are no other lists, but keep **New list** available. Never populate empty households with prototype sample lists. New list uses the shared Button’s solid, borderless warm-neutral treatment (`#F4F3EF`, ink text, 16pt Nunito ExtraBold, approximately 49pt minimum height, pill radius, 12pt top gap), not glass or a white outlined button. Retain shared reduced-motion-aware press feedback.
- Collapsing tabs must derive every icon centre and selection-pill bounds from `getTabBarSegmentGeometry`. Four tabs need four uniformly compressed segments, including the inner pair; never use edge-only shifts inherited from a three-tab layout. Clamp spring progress to keep the pill within its material.
- Use white `EmptyStateCard` surfaces for completed review, no tracked products, learning-only products and paused tracking. When nothing is due in an otherwise active Pantry, render no quiet-state card or its empty spacing. Keep completed-review feedback/Undo and setup guidance. Use existing modestly sized artwork. Do not add the prototype’s removed stocktaking, automatic-addition or Pantry-removal footnotes.
- Secondary shopping-list pages expose **Archive list** directly as an archive icon in the header while it is the only list-level option. Keep the existing confirmation and cancel action; do not archive on the header tap itself or add an overflow sheet for a single option.
- Use Gesture Handler + Reanimated for horizontal intent, UI-thread translation/rotation, velocity-aware commit and spring-back. Vertical scrolling must remain available; Reduce Motion removes card translation/rotation. Never remove the button alternative.
- Only advance a local decision after it is saved or safely queued. Failed saves leave the card available with an error. Server-backed Undo lasts five minutes, requires connectivity, checks for later product/list/item changes, and only removes untouched items created by that exact decision. Never roll back a whole household snapshot in the client. Queued offline choices explicitly have no Undo.
...
- **Dismissal:** Every single-line field uses Done/Return to blur. Tapping outside a field, dragging any scroll surface, moving to another route, or panning a sheet releases the keyboard. Multiline notes keep Return for new lines and expose the shared iOS Done accessory, while retaining the same outside-tap and scroll escape paths. New screen scroll views use `keyboardDismissScrollProps`; scrolling sheets use `GlassBottomSheetScrollView`, which owns those defaults. Do not deliberately refocus a field after submission.
...
- **Destinations:** Plan, Shop, Pantry, and Spending. Pantry is the household's learned grocery memory and the sole persistent home for tracked-product review and editing. Household settings open from the profile control and do not occupy a primary tab.

#### Tab scroll chrome

- **Shared structure:** Plan, Shop, Pantry, and Spending use `CollapsibleTabHeader` with a large semantic heading in scroll content and one compact visual title in fixed navigation chrome. Do not create screen-specific large-title animations.
...
- **Keyboard contract:** Editable sheet forms use the shared `Input` and `AmountInput`; `GlassBottomSheet` automatically supplies Gorhom's keyboard-integrated text field. Tall forms use `GlassBottomSheetScrollView`, which reveals the focused field again after the keyboard takes space, dismisses on a user drag, observes outside taps without swallowing the intended control, and remains scrollable while editing. Single-line submit dismisses reliably; multiline fields preserve line breaks and provide the shared iOS Done accessory. Do not render a raw `TextInput` inside a sheet or override the shared keyboard behaviour without platform UAT.
...
Search includes an inline clear × whenever its value is nonempty (also while blurred). Reserve its 48 pt trailing slot to avoid text-width shifts. Clearing dismisses the keyboard, empties only the query, and resets scroll without changing the selected filter. Use the shared solid ghost Button for press feedback; no delayed entrance or clearing animation.

The Pantry search field owns its native text and selection (`defaultValue`, not a controlled `value` echo from list state). Forward text changes to filtering without writing them back into the native field. Both clear × and Show all shelves use its explicit clear handle so native text, filter query and clear-control visibility reset together. Do not force selection or remount the field on each query.

Search lives below the large title in scroll content, followed by All shelves,
Learning and Paused filters. Both status filters include the household product count, including zero; counts describe the full status group, not the current search results. Keep the shared profile header and four global tabs. Tab labels use Nunito ExtraBold (`Nunito_800ExtraBold`), without synthetic font weight. Selecting a filter or Show all shelves resets Pantry to its absolute scroll origin, including the full header. Disable FlashList visible-content anchoring here; it otherwise follows a shared shelf down the expanded results. Use the post-layout reset for user actions only, not for background product updates.
...
- **Actions:** Empty-state CTAs use the shared Button with `forceSolid`. Their visibility must not depend on a native glass layer reattaching when a filtered/recycled list becomes empty again. Test empty → show all → same empty filter repeatedly; the label and action must remain visible and usable.
