...
describe("secondary screen design system", () => {
  it("keeps Pantry data controllers outside the sheet portal and has no redundant Shop CTA", () => {
    ...
    expect(pantry.indexOf("<PantryShopAction")).toBeLessThan(
      pantry.indexOf("<GlassBottomSheet\n"),
    );
  ...
  it("uses the shared progress and sheet hierarchy on list details", () => {
    ...
    expect(listDetail).toContain("<ShoppingListSummary");
    const summary = readSource("components/lists/ShoppingListSummary.tsx");
    expect(summary).toContain("<ProgressBar");
    ...
    expect(summary).toContain('size="compact"');
  ...
  it("uses calm partner activity motion", () => {
